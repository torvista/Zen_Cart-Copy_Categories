...
<?php
  ...
  switch ($action) {
    ...
    */
    ...
    $contents[] = array('text' => sprintf(TEXT_MOVE_CATEGORIES_INTRO, $cInfo->categories_name));
    $contents[] = array('text' => '<br />' . sprintf(TEXT_COPY, $cInfo->categories_name) . '<br />' . zen_draw_pull_down_menu('copy_to_category_id', zen_get_category_tree(), $current_category_id));
    $contents[] = array('align' => 'center', 'text' => '<br />' . zen_image_submit('button_copy.gif', IMAGE_COPY) . ' <a href="' . zen_href_link(FILENAME_CATEGORIES, 'cPath=' . $cPath . '&cID=' . $cInfo->categories_id) . '">' . zen_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>');
    break;
	
	//BOF COPY CATEGORY ADD-ON
 ...
 * @author Pavel.Galagan@gmail.com
 *
*/
